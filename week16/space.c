...
void our_malloc(int type, void **target, int *mem_location)
{
    ...
    if(type == TYPE_SMALL){
        ...
        if (location >= 0) {
            ...
            *target = (void *)&buffer[LARGE_START + location * LARGE_ELEMENT_SIZE];
            *mem_location = location + NUM_SMALL_BYTE_BUF; // 加上 offset
            return;
        }
        ...
        if (location >= 0) {
            set_single_bit(&byte_large_buf_mask, location);
            *target = (void *)&buffer[LARGE_START + location * LARGE_ELEMENT_SIZE];
            *mem_location = location + NUM_SMALL_BYTE_BUF;
            return;
        }
